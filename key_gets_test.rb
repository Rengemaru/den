...
class KeyGetsTest < Gosu::Window
    def initialize
      ...
      # フォーム関連
      @form_mode = false
      @current_field = 0  # 現在入力中のフィールド（0:文字, 1:色, 2:サイズ, 3:速度）
      ...
      @size_names = ["小", "中", "大", "特大"]
      @current_size_index = 1  # デフォルトは中(50)
      
      @speeds = [-1, -2, -4, -6]
      @speed_names = ["とても遅い", "遅い", "普通", "速い"]
      @current_speed_index = 1  # デフォルトは遅い(-2)
      
      @field_names = ["文字", "色", "サイズ", "速度"]
      
      # フォームの各フィールドの位置
      @field_positions = [
        { x: 150, y: 150, width: 400, height: 40 },  # 文字入力
        { x: 150, y: 220, width: 400, height: 40 },  # 色選択
        { x: 150, y: 290, width: 400, height: 40 },  # サイズ選択
        { x: 150, y: 360, width: 400, height: 40 }   # 速度選択
      ]
    ...
    def update
      ...
      @x = 800 if @x < -text_width  # 画面外に出たら右端に戻す
    end
    ...
    def draw_form
      ...
      # 各フィールドの描画
      draw_field(0, "文字:", @form_data[:text], small_font)
      draw_field(1, "色:", @color_names[@form_data[:color_index]], small_font)
      draw_field(2, "サイズ:", @size_names[@form_data[:size_index]], small_font)
      draw_field(3, "速度:", @speed_names[@form_data[:speed_index]], small_font)
    ...
    def draw_field(index, label, value, font)
      pos = @field_positions[index]
      
      # ラベル
      font.draw_text(label, 70, pos[:y] + 10, 5, 1.0, 1.0, Gosu::Color::WHITE)
      ...
      font.draw_text(display_value, pos[:x] + 10, pos[:y] + 10, 7, 1.0, 1.0, Gosu::Color::WHITE)
      
      # 選択項目には矢印表示
      if index > 0 && @current_field == index
        font.draw_text("←→", pos[:x] + pos[:width] - 40, pos[:y] + 10, 7, 1.0, 1.0, Gosu::Color::CYAN)
      end
    end
